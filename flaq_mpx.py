...
#Uses local containers on HiPerGator
'''
This program generates a consensus assembly from tile amplicon whole-genome data for monkeypox from
clinical specimens. Assumes Illumina paired-end data and requires a reference fasta, gff3, and primer scheme
bed file.
'''
...
parser = argparse.ArgumentParser(usage='flaq_mpx.py <input_dir> [options]')
parser.add_argument('input', help='path to dir with fastqs')
parser.add_argument('--primer_bed', help='path to primer bed file', required=True)
...
parser.add_argument('--version', action='version', version='This is flaq_mpx: Version 1.0.0', help='print version')
...
#header = ['sampleID', 'reference', 'start', 'end', 'num_raw_reads', 'num_clean_reads', 'num_mapped_reads', 'percent_mapped_clean_reads', 'cov_bases_mapped', 'percent_genome_cov_map', 'mean_depth', 'mean_base_qual', 'mean_map_qual', 'assembly_length', 'numN', 'percent_ref_genome_cov', 'VADR_flag', 'QC_flag', 'nextclade']
header = ['sampleID', 'reference', 'start', 'end', 'num_raw_reads', 'num_clean_reads', 'num_mapped_reads', 'percent_mapped_clean_reads', 'cov_bases_mapped', 'percent_genome_cov_map', 'mean_depth', 'mean_base_qual', 'mean_map_qual', 'assembly_length', 'numN', 'percent_ref_genome_cov', 'QC_flag']
...
for s in samples:
    ...
    subprocess.run('cd ' + var_path + ' && singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.12.sif samtools mpileup -A -d 8000 --reference ' + ref + ' -B -Q 0 ' + align_dir + s + '.primertrim.sorted.bam | singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/ivar_1.3.1.sif ivar variants -r ' + ref + ' -m 10 -p ' + s + '.variants -q 20 -t 0.25 -g ' + gff, shell=True, stdout=out_log, stderr=err_log, check=True)

    #Generate consensus assembly
    assem_path = sample_dir + 'assembly/'
    subprocess.run('mkdir ' + assem_path, shell=True, check=True)
    subprocess.run('cd ' + assem_path + ' && singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/samtools_1.12.sif samtools mpileup -A -B -d 8000 --reference ' + ref + ' -Q 0 ' + align_dir + s + '.primertrim.sorted.bam | singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/ivar_1.3.1.sif ivar consensus -t 0 -m 10 -n N -p ' + s + '.consensus', shell=True, stdout=out_log, stderr=err_log, check=True)
    ...
    if qc_flag == 'PASS':
        ...
        #Run nextclade
        #subprocess.run('singularity exec -B $(pwd):/data /apps/staphb-toolkit/containers/nextclade_2021-03-15.sif nextclade --input-fasta ' + assem_path + s + '.consensus.fa --output-csv ' + assem_path + 'nextclade_report.csv --jobs ' + threads, shell=True, check=True)

    #Write to output file
    #results = [s, ref_name, start, end, raw_reads, clean_reads, reads_mapped, percent_map, cov_bases, cov, depth, baseq, mapq, num_bases, ns, pg, vadr_flag, qc_flag, clade]
    results = [s, ref_name, start, end, raw_reads, clean_reads, reads_mapped, percent_map, cov_bases, cov, depth, baseq, mapq, num_bases, ns, pg, qc_flag]
...
#Create multi-fasta file of only passing assemblies
